...
const RegisterPage = () => {
  ...
  const { refreshProfile, user, loading: authLoading, profile } = useAuth();
  ...
  useEffect(() => {
    if (user && !authLoading && !isSubmitting.current) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Route protection handles profile completion redirection automatically
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const user = await signUp(data.username, data.password, data.name);
        ...
        <div className="glass-dark p-8 sm:p-10">
          ...
          >
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    {...register("password")}
                    className="w-full bg-white/5 border border-white/10 rounded-xl pl-4 pr-12 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-red-500/60 transition-all text-sm"
                  />
                  <button
                    type="button"
            ...
            <button
              ...
              type="submit"
              disabled={loading}
              className="btn-LifeDrop w-full py-3 rounded-xl font-semibold md:text-lg text-sm"
            >
